  ...
  <div class="page">
    ...
    <section class="result" v-if="phase === 'result' && display">
      ...
      <div class="result-actions">
        ...
        <button class="act" :class="{ primary: !canSend }" @click="handleDownload">⬇ 下载预览图</button>
...
async function onGenerate() {
  ...
  const outcome = await runUntilImage(
    ...
    (result) => templateResultToDisplay(result, '宠物说话预览'),
    () => phase.value === 'loading',
  )
  ...
  phase.value = 'form'
  showToast('当前生成人数较多，已为你保留台词，请稍后再试一次')
}
...
onMounted(async () => { configure(await fetchAdConfig('/tg/pet-talk')) })
function handleDownload() {
  if (!display.value) { showToast('还没有可下载的图片'); return }
  requestDownload(async () => {
    const r = await downloadImage(display.value!.src, 'pet-talk-' + Date.now() + '.png')
